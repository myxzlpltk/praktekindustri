...
class PdfmakerController extends Controller{
	...
	public function lembarSahProposalPKRN(Request $request){
		...
		$yNama = $fpdf->getY();
		$fpdf->setX($xKajur);
		$fpdf->Cell(40, 6, 'Aji Prasetya Wibawa, S.T., M.M.T., Ph.D.');
		$fpdf->setXY($xKajur, $yNama+6);
		$fpdf->Cell(40, 6, 'NIP 197912182005011001');

		$fpdf->SetXY($xKoor, $yNama);
		if(in_array($request->prodi, [2, 4, 5])){
			$fpdf->Cell(40,6, 'Kartika Candra Kirana, S.Pd., M. Kom.');
			$fpdf->setXY($xKoor, $fpdf->getY()+6);
